...
class WALI(nn.Module):
    def __init__(self, E, G, C,loss_type='mse'):
    ...
    def criticize(self, z_hat,z):
        # input_x = torch.cat((x, x_tilde), dim=0)
        input_z = torch.cat((z,z_hat), dim=0)
        output = self.C(input_z)
        data_preds, sample_preds = output[:z.size(0)], output[z.size(0):]
        return data_preds, sample_preds

    def calculate_grad_penalty(self, z_hat,z):
        bsize = z_hat.size(0)
        eps = torch.rand(bsize, 1, 1, 1).to(z_hat.device)  # eps ~ Unif[0, 1]
        # zintp_x = eps * x + (1 - eps) * x_tilde
        intp_z = eps * z_hat + (1 - eps) * z
        # intp_x.requires_grad = True
        intp_z.requires_grad = True
        tmp_C = self.C(intp_z)
        C_intp_loss = tmp_C.sum()
        grads = autograd.grad(C_intp_loss, (intp_z), retain_graph=True, create_graph=True)
        grads_z = grads[0].view(bsize, -1)
        # grads = torch.cat((grads_x, grads_z), dim=1)
        grad_penalty = ((grads_z.norm(2, dim=1) - 1) ** 2).mean()
        return grad_penalty

    def forward(self, x, z, lamb=10,beta1=0.01,beta2=0.01,beta3=0.03,gan=0,loss_type='mse',var_beta=-1,clip_beta=-1,methods=0,l1=True,val_range=2,normalize="relu",pads=False,ssm_alpha=0.84):
        ...
        if methods > 0:
            z_hat, x_tilde = self.encode(x), self.reconstruct(x)
        else:
            z_hat, x_tilde = self.encode(x), self.reconstruct(x)

        self.loss_type = loss_type

        if self.loss_type.strip() == 'mse'.strip():
            # print(self.loss_type.strip() == 'mse'.strip())
            recon_loss = F.mse_loss(x_tilde, x,reduction='sum')/x.size()[0]
        elif self.loss_type.strip() == 'l1'.strip():
            recon_loss = F.l1_loss(x_tilde,x,reduction='sum')/x.size()[0]
        elif self.loss_type.strip() == 'cross'.strip():
            recon_loss = F.binary_cross_entropy_with_logits(input=x_tilde,target=x,reduction='sum')/x.size()[0]
        else:
            # print(self.loss_type.strip() == 'mse'.strip())
            recon_loss = F.mse_loss(x_tilde, x, reduction='sum') / x.size()[0]
        data_preds, sample_preds = self.criticize(z_hat, z)
        EG_loss = torch.mean(data_preds - sample_preds)
        C_loss = -EG_loss + lamb * self.calculate_grad_penalty(z_hat.data, z.data)

        return recon_loss,EG_loss,C_loss
