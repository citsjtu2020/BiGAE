...
class Generator_with_drop(nn.Module):
    def __init__(self,gpu_mode,shift=None):
        ...
        self.ReLU5 = LeakyReLU(LEAK, inplace=True)
        self.ConvT6 = ConvTranspose2d(DIM * 4, DIM*4, 4, 2, 1, bias=False)
        self.B6 = BatchNorm2d(DIM*4)
        self.ReLU6 = LeakyReLU(LEAK, inplace=True)
        self.ConvT7 = ConvTranspose2d(DIM *4, DIM*2, 4, 2, 1, bias=False)
        self.B7 = BatchNorm2d(DIM*2)
        self.ReLU7 = LeakyReLU(LEAK, inplace=True)
        self.ConvT8 = ConvTranspose2d(DIM * 2, DIM, 4, 2, 1, bias=False)
        self.B8 = BatchNorm2d(DIM)
        self.ReLU8 = LeakyReLU(LEAK, inplace=True)
        self.ConvT9 = ConvTranspose2d(DIM, NUM_CHANNELS, 4, 2, 1, bias=False)
        # ConvTranspose2d()
        # self.ConvT1 = ConvTranspose2d(NLAT, DIM * 16, 4, 1, 0, bias=False)
        # self.B1 = BatchNorm2d(DIM*16)
        # self.ReLU1 = LeakyReLU(inplace=True,negative_slope=LEAK)
...
# class Generator_with_drop(nn.Module):
    ...
    def forward(self, input,var_beta=0.1,clip_beta=0.2):
        ...
        #1024
        out8 = self.ReLU8(self.B8(self.ConvT8(out7)))
        out9 = self.ConvT9(out8)
        output = self.act(out9)
...
def ceshis():
    x_mapping = nn.Sequential(
        ...
        Conv2d(DIM * 8, DIM * 8, 4, 2, 1), LeakyReLU(LEAK),
        Conv2d(DIM * 8, DIM * 8, 4, 2, 1), LeakyReLU(LEAK),
        Conv2d(DIM * 8, DIM * 16, 4, 1, 0), LeakyReLU(LEAK)
    )
    return DeterministicConditional(x_mapping)
...
#
    ...
    if local_rank == 0:
        ...
        # summary(ces,(NUM_CHANNELS,IMAGE_SIZE,IMAGE_SIZE))
#
...
#
            ...
            if EG_update1:
                ...
                optimizerE2.zero_grad()
                C_loss, EG_loss, mss_loss, l1_conv, mss_x_loss, EG_loss2, RECON_Z_loss, RECON_X_loss = wali.forward(x=x,z=z,lamb=LAMBDA,gan=2,loss_type='msssim',beta1=0.2,beta2=0.28,beta3=0.72,methods=0,l1=True,l2=False,val_range=2,normalize="relu", pads=False,ssm_alpha=0.675)
                RECON_Z_loss2 = RECON_Z_loss*RECON_lamb*0.048
...
#             # print training statistics
            ...
            if curr_iter % 250 == 0 and local_rank == 0:
                wali.eval()
...
#
            # save model
            if curr_iter % (ITER // 50) == 0 and local_rank==0:
