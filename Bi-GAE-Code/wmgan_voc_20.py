...
def main():
    mmds = os.path.join(save_path, 'mmds-tanh%d' % IMAGE_SIZE)
    if not os.path.exists(os.path.join(save_path, 'mmds-tanh%d' % IMAGE_SIZE)):
        os.makedirs(os.path.join(save_path, 'mmds-tanh%d' % IMAGE_SIZE))
        print("目录创建成功！")
    ...
    while curr_iter < ITER and epoch <= EPOCH:
        ...
        for batch_idx, (x, labels) in enumerate(train_loader):
            ...
            if curr_iter % 200 == 0:
                ...
                if not TANH:
                    ...
                    rect_imgs[:, 1, :, :] = rect_imgs[:, 1, :, :] * 0.225 + 0.406

                    genr_imgs[:, 0, :, :] = genr_imgs[:, 0, :, :] * 0.229 + 0.485
                    genr_imgs[:, 1, :, :] = genr_imgs[:, 1, :, :] * 0.224 + 0.456
                    genr_imgs[:, 1, :, :] = genr_imgs[:, 1, :, :] * 0.225 + 0.406
            ...
            if curr_iter % (ITER // 20) == 0:
                if torch.cuda.device_count() > 1:
                    ...
                    torch.save(wali._modules['module'].state_dict(),
                               mmds + '/voc/%d/total_%d.pth' % (num_exp, curr_iter))

                else:
                    torch.save(wali, mmds + '/voc/%d/save_model_%d.ckpt' % (num_exp, curr_iter))
                    torch.save(wali.state_dict(), mmds + '/voc/%d/total_%d.pth' % (num_exp, curr_iter))
        ...
        if torch.cuda.device_count() > 1:
            ...
            torch.save(wali._modules['module'].state_dict(), mmds + '/voc/%d/total_epoch_%d.pth' % (num_exp, epoch))

        else:
            torch.save(wali, mmds + '/voc/%d/save_model_epoch_%d_%d.ckpt' % (num_exp, epoch, curr_iter))
            torch.save(wali.state_dict(), mmds + '/voc/%d/total_epoch_%d.pth' % (num_exp, epoch))
