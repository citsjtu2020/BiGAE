...
class WALI(nn.Module):
    def __init__(self, E, G, C,MMDX,MMDZ,rep_weights=[0.0,-1.0],mmd_g_scale=0.1,loss_type='mmd0',sigma=[1,np.sqrt(2),2,np.sqrt(8),4]):
    ...
    def forward(self, x, z, lamb=10,beta1=0.01,beta2=0.01,gan=0,loss_type='raw',var_beta=-1,clip_beta=-1,methods=0):
        ...
        if gan == 0:
            ...
            if self.loss_type == 'mmd':
                ...
                RECON_Z_loss = beta2*RECON_Z_loss
                ...
                RECON_Z_loss = F.mse_loss(input=z_recon,target=z_hat,reduction='mean')
                # RECON_Z_loss = F.mse_loss(input=z_recon,target=z_hat,reduction='sum')/batch_sizes
                RECON_Z_loss = RECON_Z_loss*beta2
                EG_loss2 = EG_loss+beta1*RECON_Z_loss
                return C_loss,EG_loss,RECON_Z_loss,EG_loss2,mmd_penalty
            else:
                if methods > 0:
