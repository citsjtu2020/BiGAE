...
def get_squared_dist(
    ...
    if mode in ['xx', 'xxxy', 'xxxyyy']:
        ...
        if torch.cuda.is_available():
            device = dx.get_device()
            dist_xy = torch.max(torch.unsqueeze(dx, dim=1) - 2.0 * xyt + torch.unsqueeze(dy, dim=0), torch.Tensor([0]).cuda(device))
        else:
...
def mmd_g(
    ...
    if lower_bound is None:
        ...
        if torch.cuda.is_available():
            ...
            k_xx = torch.exp(-torch.max(dist_xx, torch.Tensor([lower_bound]).cuda(device)) / (2.0 * sigma ** 2))
            k_yy = torch.exp(-torch.max(dist_yy, torch.Tensor([lower_bound]).cuda(device)) / (2.0 * sigma ** 2))
        else:
    ...
    if upper_bound is None:
        ...
        if torch.cuda.is_available():
            ...
            k_xy = torch.exp(-torch.min(dist_xy, torch.Tensor([upper_bound]).cuda(device)) / (2.0 * sigma ** 2))
        else:
...
def mmd_g_bounded(
    ...
    if torch.cuda.is_available():
        device = dist_xx.get_device()
        k_xx_b = torch.exp(-torch.max(dist_xx, torch.Tensor([lower_bound]).cuda(device))/(2.0*sigma**2))
    else:
    ...
    if custom_weights[0] > 0:
        if torch.cuda.is_available():
            device = dist_xy.get_device()
            k_xy_b = torch.exp(-torch.min(dist_xy,torch.Tensor([upper_bound]).cuda(device)) / (2.0 * sigma ** 2))
        else:
    ...
    if custom_weights[1] > 0:  # the original mmd-g
        if torch.cuda.is_available():
            ...
            k_yy_b = torch.exp(-torch.max(dist_yy, torch.Tensor([lower_bound]).cuda(device)) / (2.0 * sigma ** 2))
        else:
        ...
        if torch.cuda.is_available():
            device = dist_yy.get_device()
            k_yy_b = torch.exp(-torch.min(dist_yy, torch.Tensor([upper_bound]).cuda(device)) / (2.0 * sigma ** 2))
        else:
...
def rand_mmd_g_xy(dist_xx, dist_xy, dist_yy, batch_size=None, dist_yx=None, omega=0.5, max_iter=3, name='mmd',
    ...
    if dist_yx is None:
        ...
        if torch.cuda.is_available():
            device = dist_xy.get_device()
            beta = -torch.log(torch.tensor(omega).cuda(device)) / torch.mean(dist_xy + EPSI)  # beta = 1/2/sigma
        else:
        ...
        if torch.cuda.is_available():
            device = dist_xy.get_device()
            beta = -2.0 * torch.log(torch.tensor(omega).cuda(device)) / (torch.mean(dist_xy) + torch.mean(dist_yx) + EPSI)
        else:
...
def rand_mmd_g_xn(
    ...
    if dist_xx is None:
        ...
        if torch.cuda.is_available():
            device = dx.get_device()
            dist_xx = torch.max(torch.unsqueeze(dx, dim=1) - 2.0 * xxt + torch.unsqueeze(dx, dim=0), torch.Tensor([0.0]).cuda(device))
        else:
    ...
    def f(b):
        ...
        if torch.cuda.is_available():
            device = dist_xy.get_device()
            k = torch.pow(const_f, torch.tensor(d / 2.0).cuda(device)) * torch.exp(-b * const_f * dist_xy)
        else:
    ...
    def df(k):
        ...
        if torch.cuda.is_available():
            device = dist_xy.get_device()
            kd = -y_rho * k[0] / 2.0 * k[2] - torch.mean(torch.pow(k[0], torch.tensor(2).cuda(device)) * dist_xy * k[1])  # gradient of exp(-d*w)
        else:
    ...
    if torch.cuda.is_available():
        device = dist_xy.get_device()
        beta = -torch.log(torch.tensor(omega).cuda(device)) / (torch.mean(dist_xy) + y_rho / 2.0)  # beta = 1/2/sigma
    else:
    ...
    if torch.cuda.is_available():
        device = dist_xy.get_device()
        k_xy = torch.pow(const_0, torch.tensor(d / 2.0).cuda(device)) * torch.exp(-beta * const_0 * dist_xy)
    else:
    ...
    if torch.cuda.is_available():
        ...
        e_kyy = torch.pow(d / (d + 2.0 * beta * y_rho), torch.tensor(d / 2.0).cuda(device))
    else:
    ...
    def _rand_g_instance_noise_(self,s_recon, s_x, mix_threshold=0.2,omega_range = [0.05, 0.85],ref_normal = 1.0,num_scores=None):
        ...
        if self.sig is None:
            if torch.cuda.is_available():
                ...
                self.sig = torch.tensor(0).cuda(device)
            else:
