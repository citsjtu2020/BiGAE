...
from util5 import DeterministicConditional, GaussianConditional, JointCritic, WALI,MMD_NET
...
MMD_ITERS = 3
LAMBDA = 10       # strength of gradit penalty
LEARNING_RATE = 1e-5
BETA1 = 0.5
BETA2 = 0.9
...
class CifarDataset(data.Dataset):
  def __init__(self, aims):
    ...
    self.aims = aims
    self.total_data = np.load("class_cifar.npy",allow_pickle=True).item()

    if isinstance(aims, list):
      # for a in aims:
      #   data
      # self.datas = self.total_data[]
      data0 = self.total_data[aims[0]]
      # print(data0)
      for a in range(1,len(aims)):
        tmp_data = self.total_data[aims[a]]
        # data0 = np.vstack((data0,tmp_data))
        data0 = np.vstack((data0, tmp_data))
      # data0 =
      np.random.shuffle(data0)
    else:
      if aims < 0:
        data0 = self.total_data[0]
        # print(data0.shape)
        for a in range(1,10):
          tmp_data = self.total_data[a]
          data0 = np.vstack((data0,tmp_data))
        np.random.shuffle(data0)

      else:
        data0 = self.total_data[aims]
    print(data0.shape)
    self.datas = data0[:,:-1]
    self.labels = data0[:,-1]
  ...
  def __getitem__(self, item):
    ...
    item_data = self.datas[item,:]
    # print(item_data)
    item_label = self.labels[item]
    # print(type(item_label))
    item_data = (item_data - 0.5)/0.5
    ...
    data = torch.reshape(data,[3,IMAGE_SIZE,IMAGE_SIZE])
...
def create_WALI():
  ...
  wali = WALI(E, G, C,MMDX,MMDZ,channel=3,pads=False,val_range=2)
  return wali
...
def main():
  ...
  mmds = os.path.join(data_path, 'cifar')
  ...
  optimizerXM = Adam(list(wali.get_C().get_x_net_parameters()) + list(wali.get_mmdx_parameters()), lr=LEARNING_RATE,
                     betas=(BETA1, BETA2))
  optimizerZM = Adam(list(wali.get_C().get_z_net_parameters()) + list(wali.get_mmdz_parameters()), lr=LEARNING_RATE,
                     betas=(BETA1, BETA2))
  optimizerXZM = Adam(list(wali.get_C().get_x_net_parameters()) + list(wali.get_C().get_z_net_parameters()) + list(
  ...
  while curr_iter < ITER:
    for batch_idx, (x, y) in enumerate(loader, 1):
      ...
      # if MMD_UPDATE:
      #   optimizerMox.zero_grad()
      #   C_loss, EG_loss, RECON_Z_loss, EG_loss2, mmd_penalty = wali.forward(x=x, z=z, lamb=LAMBDA, beta1=1.1, beta2=0.12,
      #                                                                       gan=1, loss_type='rmb_gp')
      #   C_loss.backward()
      #   optimizerMox.step()
      #   # print("MMD update")
      #   MMD_iter += 1
      ...
      # if EG_update2:
      #   optimizerEG2.zero_grad()
      #   C_loss, EG_loss = wali.forward(x=x, z=z, lamb=LAMBDA, gan=0, loss_type='raw')
      #   C_losses.append(C_loss.item())
      #   EG_losses.append(EG_loss.item())
      #   C_loss, EG_loss, RECON_Z_loss, EG_loss2, mmd_penalty = wali.forward(x=x, z=z, lamb=LAMBDA, beta1=0.128,
      #                                                                       beta2=0.084, gan=1, loss_type='rmb_gp')
      #   EG_loss2.backward()
      #   MMD_C_losses.append(C_loss.item())
      #   MMD_EG_losses.append(EG_loss.item())
      #   Recon_z_losses.append(RECON_Z_loss.item())
      ...
      if curr_iter % 10 == 0:
        # print(EG_loss2)
        print(
          '[%d/%d]\tW-distance: %.4f\tC-loss: %.4f\tMMD-EG-loss: %.4f\tMMD-C-loss: %.4f\treconz-loss: %.4f\tmmd_penalty:%.4f\ttotal_gen-loss:%.4f'
          % (curr_iter, ITER, EG_losses[-1], C_losses[-1], MMD_EG_losses[-1], MMD_C_losses[-1], Recon_z_losses[-1],
             mmd_penaltys[-1],
             EG_losses2[-1]))
  ...
  # np.save("mmds/alphas9.npy",alphas)
  # EG_losses = np.load("/home/qian/huaqin/tfdata/cifar/%d/EGloss.npy" % num_exp)
  # C_losses = np.load("/home/qian/huaqin/tfdata/cifar/%d/Closs.npy" % num_exp)
  ...
  #       torch.save(wali, 'cifar10/models/total_%d.pth' % curr_iter)
  #       torch.save(wali.get_G(), 'cifar10/models/G_%d.pth' % curr_iter)
  #       torch.save(wali.get_C(), 'cifar10/models/C_%d.pth' % curr_iter)
  #       torch.save(wali.get_E(), 'cifar10/models/E_%d.pth' % curr_iter)
  # # plot training loss curve
