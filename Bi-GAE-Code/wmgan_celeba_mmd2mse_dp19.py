...
from util94 import DeterministicConditional, GaussianConditional, JointCritic, WALI,MMD_NET
...
def main():
    mmds = os.path.join(save_path,'mmds-mmd2mse%d' % IMAGE_SIZE)
    if not os.path.exists(os.path.join(save_path,'mmds-mmd2mse%d' % IMAGE_SIZE)):
        os.makedirs(os.path.join(save_path,'mmds-mmd2mse%d' % IMAGE_SIZE))
        print("目录创建成功！")
    ...
    # save_model_epoch_88_4495.ckpt
    now_model = torch.load("/data1/JCST/results/mmds-mmd2mse256/celeba/22/total_epoch_88.pth", map_location="cuda:0")
...
#
            ...
            if curr_iter % (ITER // 40) == 0:
                ...
                np.save(mmds + "/%d/Recon_z_loss2.npy" % num_exp, Recon_z_losses2)
