...
import numpy as np
import math_util2
from math_util2 import  witness_g,witness_mix_g,get_squared_dist,jacobian_squared_frobenius_norm
...
class WALI(nn.Module):
    def __init__(self, E, G, C,MMDX,MMDZ,rep_weights=[0.0,-1.0],mmd_g_scale=0.1,loss_type='msssim',sigma=[1,np.sqrt(2),2,np.sqrt(8),4],window_size=11,size_average=True,val_range=2,channel=3,l1=True,pads=False,loss_type2=None):
    ...
    def forward(self, x, z, lamb=10,beta1=0.01,beta2=0.01,beta3=0.03,gan=0,loss_type='raw',var_beta=-1,clip_beta=-1,methods=0,l1=True,val_range=2,normalize="relu",pads=False,ssm_alpha=0.84):
        ...
        if gan == 0:
            ...
            if self.loss_type == 'mmd':
                ...
                C_loss,EG_loss = math_util2.mmd_g_loss(s_recon,s_z,sigma=self.sigma,dis_penalty=mmd_penalty)
                # z_recon2 = self.encode(x_tilde_copy)
                # # RECON_Z_loss = F.mse_loss(input=z_recon,target=z_hat,reduction='sum')/batch_sizes
                # # RECON_Z_loss = F.mse_loss(input=z_recon2,target=z_hat,reduction='mean')
                # RECON_Z_loss = F.mse_loss(input=z_recon2, target=z, reduction='mean')
                ...
                mmd_penalty = self.mmd_gradient_penalty(z, z_recon,aim=1)
                mmd_penalty = lamb * mmd_penalty
                C_loss,EG_loss = math_util2.mmd_g_bound_loss(s_recon,s_x,mmd_penalty)
                # z_recon2 = self.encode(x_tilde_copy)
                # # RECON_Z_loss = F.mse_loss(input=z_recon,target=z_hat,reduction='sum')/batch_sizes
                # RECON_Z_loss = F.mse_loss(input=z_recon2,target=z,reduction='mean')
                C_loss = beta1*C_loss
                ...
                mmd_penalty = lamb * mmd_penalty
                C_loss,EG_loss = math_util2.mmd_repulsive_g_loss(s_recon,s_x,repulsive_weights=self.rep_weights,
                ...
                mmd_penalty = lamb*mmd_penalty
                C_loss,EG_loss = math_util2.mmd_repulsive_g_bounded_loss(s_recon,s_x,self.rep_weights,mmd_penalty)
                C_loss = beta1*C_loss
                EG_loss = beta1*EG_loss
                # z_recon2 = self.encode(x_tilde_copy)
                # RECON_Z_loss = F.mse_loss(input=z_recon2,target=z,reduction='mean')
                ...
                mmd_penalty = self.mmd_gradient_penalty(z, z_recon,aim=1)
                mmd_penalty = lamb * mmd_penalty
                C_loss, EG_loss = math_util2.mmd_g_loss(s_recon, s_x, sigma=self.sigma, dis_penalty=mmd_penalty)
                z_recon2 = self.encode(x_tilde_copy)
                # RECON_Z_loss = F.mse_loss(input=z_recon, target=z_hat, reduction='sum') / batch_sizes
