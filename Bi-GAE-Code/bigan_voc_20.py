...
def main():
    ...
    # if local_rank == 0:
    mmds = os.path.join(save_path,'bigan-tanh%d' % IMAGE_SIZE)
    if not os.path.exists(os.path.join(save_path,'bigan-tanh%d' % IMAGE_SIZE)):
        os.makedirs(os.path.join(save_path,'bigan-tanh%d' % IMAGE_SIZE))
        print("目录创建成功！")
...
#
    ...
    # save_model_epoch_24_20855.ckpt
    ...
    # curr_iter = 20855
    #
    # init_x = np.load(mmds + "/%d/samples.npy" % 28)
