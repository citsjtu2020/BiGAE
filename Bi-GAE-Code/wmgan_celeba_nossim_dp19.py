...
from util95 import DeterministicConditional, GaussianConditional, JointCritic, WALI,MMD_NET
...
def main():
    ...
    mmds = os.path.join(save_path,'mmds-nossim%d' % IMAGE_SIZE)
    if not os.path.exists(os.path.join(save_path,'mmds-nossim%d' % IMAGE_SIZE)):
        os.makedirs(os.path.join(save_path,'mmds-nossim%d' % IMAGE_SIZE))
        print("目录创建成功！")
...
#
    ...
    now_model = torch.load("/data1/JCST/results/mmds-nossim256/celeba/22/total_epoch_85.pth", map_location="cuda:0")
...
#
            ...
            if C_update:
                ...
                C_loss, EG_loss = wali.forward(x=x,z=z,lamb=LAMBDA,gan=0,loss_type='raw',beta1=0.2,beta2=0.36,beta3=0.72,methods=0,l1=False,l2=True,val_range=2,normalize="relu", pads=False,ssm_alpha=0.675)
                # print(C_loss)
                C_loss = C_loss.mean()
            ...
            if EG_update1:
                ...
                C_loss, EG_loss, mss_loss, l1_conv, mss_x_loss, EG_loss2, RECON_Z_loss, RECON_X_loss = wali.forward(x=x,z=z,lamb=LAMBDA,gan=2,loss_type='msssim',beta1=0.2,beta2=0.35,beta3=0.72,methods=0,l1=True,l2=False,val_range=2,normalize="relu", pads=False,ssm_alpha=0.675)
