...
class Generator_with_drop(nn.Module):
    def __init__(self,gpu_mode,shift=None):
        ...
        self.B1 = BatchNorm2d(DIM * 8)
        self.ReLU1 = LeakyReLU(inplace=True, negative_slope=LEAK)
        self.ConvT2 = ConvTranspose2d(DIM * 8, DIM * 8, 4, 2, 1, bias=False)
...
def create_mmds():
    mmd_x = nn.Sequential(
        ...
        #128
        Conv2d(DIM * 8, DIM * 8, 1, 1, 0), LeakyReLU(LEAK),
        Conv2d(DIM * 8, DIM * 4, 1, 1, 0), LeakyReLU(LEAK),
        Conv2d(DIM * 4, DIM * 4, 1, 1, 0), LeakyReLU(LEAK),
        Conv2d(DIM * 4, 16, 1, 1, 0)
...
print("Batch size: % d" % (BATCH_SIZE//num_gpu))
def create_WALI():
    if torch.cuda.is_available():
...
#     # sign_
    ...
    num_dec = 0
    # print(wali.modules())
    for k in wali._modules.items():
        print("first modules:")
        # print(len(wali.modules()))
        print(k)
        break
    # # print(wali.state_dict())
    # for k in wali.state_dict():
    #     print(k)
    #     break
    # print(len(wali.parameters().items()))
    # params = list(model.named_parameters())
    for k in list(wali.named_parameters()):
        print("named first parameters:")
        print(k[0])
        print(k[1])
        break

    for k in wali.parameters():
    ...
    if torch.cuda.device_count() > 1:
        ...
        # for name,k in wali._modules.items():
        #     print("distributed modules:")
        #     print(name)
        #     # print(k.parameters())
        #     for name0,d in k._modules.items():
        #         print(name0)
        #         print(d)
        #         break
        #     print(k)
        ...
        for k in list(wali.named_parameters()):
            print("named distributed parameters:")
            print(k[0])
            print(k[1])
            break

        for k in list(wali._modules['module'].named_parameters()):
            print("modules in named distributed parameters:")
            print(k[0])
            print(k[1])
            break

        print(type(wali._modules['module']))
        for k in wali._modules['module'].get_encoder_parameters():
            print("modules in distributed parameters:")
            print(k.shape)
            print(k)
            break
        ...
        optimizerEG2 = Adam(list(wali._modules['module'].get_encoder_parameters()),
                            lr=LEARNING_RATE * 0.25, betas=(BETA1, BETA2))
        optimizerEG3 = Adam(list(wali._modules['module'].get_encoder_parameters()) + list(wali._modules['module'].get_generator_parameters()),
                            lr=LEARNING_RATE * 1.35, betas=(BETA1, BETA2))
        ...
        optimizerXM = Adam(list(wali._modules['module'].get_C().get_x_net_parameters()) + list(wali._modules['module'].get_mmdx_parameters()),
                           lr=LEARNING_RATE, betas=(BETA1, BETA2))
        optimizerZM = Adam(list(wali._modules['module'].get_C().get_z_net_parameters()) + list(wali._modules['module'].get_mmdz_parameters()),
                           lr=LEARNING_RATE, betas=(BETA1, BETA2))
        optimizerXZM = Adam(
            list(wali._modules['module'].get_C().get_x_net_parameters()) + list(wali._modules['module'].get_C().get_z_net_parameters()) + list(
                wali._modules['module'].get_mmdx_parameters()))
        optimizerMox = Adam(list(wali._modules['module'].get_mmdx_parameters()), lr=LEARNING_RATE * 0.5, betas=(BETA1, BETA2))
        optimizerMoz = Adam(list(wali._modules['module'].get_mmdz_parameters()), lr=LEARNING_RATE * 0.5, betas=(BETA1, BETA2))
    else:
        optimizerEG = Adam(
            list(wali.get_encoder_parameters()) + list(wali.get_generator_parameters()),
            lr=LEARNING_RATE, betas=(BETA1, BETA2))
        # optimizerEG2 = Adam(list(wali.get_encoder_parameters()) + list(wali.get_generator_parameters()),
        #                    lr=LEARNING_RATE*0.25, betas=(BETA1, BETA2))
        ...
        optimizerXM = Adam(
            list(wali.get_C().get_x_net_parameters()) + list(wali.get_mmdx_parameters()),
            lr=LEARNING_RATE, betas=(BETA1, BETA2))
        optimizerZM = Adam(
            list(wali.get_C().get_z_net_parameters()) + list(wali.get_mmdz_parameters()),
            lr=LEARNING_RATE, betas=(BETA1, BETA2))
        optimizerXZM = Adam(
            list(wali.get_C().get_x_net_parameters()) + list(
                wali.get_C().get_z_net_parameters()) + list(
                wali.get_mmdx_parameters()))
        optimizerMox = Adam(list(wali.get_mmdx_parameters()), lr=LEARNING_RATE * 0.5,
                            betas=(BETA1, BETA2))
        optimizerMoz = Adam(list(wali.get_mmdz_parameters()), lr=LEARNING_RATE * 0.5,
                            betas=(BETA1, BETA2))
    epoch = 0
    while curr_iter < ITER:
        print(len(loader))
...
#
#             if curr_iter % (ITER // 40) == 0 and local_rank == 0:
