...
def main():
    ...
    mmds = os.path.join(save_path,'mmds%d' % IMAGE_SIZE)
    if not os.path.exists(os.path.join(save_path,'mmds%d' % IMAGE_SIZE)):
        os.makedirs(os.path.join(save_path,'mmds%d' % IMAGE_SIZE))
        print("目录创建成功！")
...
#
    ...
    if torch.cuda.device_count() > 1:
        ...
        #                    lr=LEARNING_RATE * 0.36, betas=(BETA1, BETA2))
...
#
            ...
            if C_update:
                ...
                results = wali.forward(x=x,z=z,lamb=LAMBDA,gan=0,loss_type='raw',beta1=0.2,beta2=0.28,beta3=0.72,methods=0,l1=False,l2=True,val_range=2,normalize="relu", pads=False,ssm_alpha=0.675)
                ...
                if MMD_iter < MMD_ITERS:
                    results = wali.forward(x=x, z=z, lamb=LAMBDA, beta1=0.8, beta2=0.62, beta3=1.0,
            ...
            if EG_update1:
                ...
                optimizerEG3.zero_grad()
                # optimizerE2.zero_grad()
                results = wali.forward(x=x,z=z,lamb=LAMBDA,gan=2,loss_type='msssim',beta1=0.2,beta2=0.28,beta3=0.72,methods=0,l1=True,l2=False,val_range=2,normalize="relu", pads=False,ssm_alpha=0.675)
                ...
                results = wali.forward(x=x,z=z,lamb=LAMBDA,beta1=0.34,beta2=0.12,gan=1,loss_type='reg-gp',methods=1,var_beta=var_beta,clip_beta=clip_beta)
            ...
            if curr_iter % 400 == 0 and local_rank == 0:
                wali.eval()
...
#
            ...
            if curr_iter % (ITER // 20) == 0 and local_rank==0:
                if torch.cuda.device_count() > 1:
                    ...
                    torch.save(wali.module, mmds + '/celeba/%d/save_model_epoch_%d.ckpt' % (num_exp, epoch))
