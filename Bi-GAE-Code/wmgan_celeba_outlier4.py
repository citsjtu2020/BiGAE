...
data_path = '/home/qian/huaqin/tfdata/celeba'
...
def create_WALI():
    ...
    wali = WALI(E, G, C,MMDX,MMDZ,window_size=5,size_average=True,val_range=2,l1=True,pads=False)
    return wali
import os

def main():
    # if not os.path.exists
    mmds = os.path.join(data_path,'mmds')
    if not os.path.exists(os.path.join(data_path,'mmds')):
        os.makedirs(os.path.join(data_path,'mmds'))
        print("目录创建成功！")
...
#     # svhn = datasets.CIFAR10('data/CIFAR10', train=True, transform=transform)
    loader = data.DataLoader(svhn, BATCH_SIZE, shuffle=True, num_workers=4)
    noise = torch.randn(32, NLAT, 1, 1, device=device)
#
...
#
            if C_update:
                ...
                C_loss,EG_loss,mss_loss,l1_conv,mss_x_loss,EG_loss2,RECON_Z_loss = wali.forward(x=x,z=z,lamb=LAMBDA,gan=0,loss_type='msssim',beta1=1.25,beta2=0.3,beta3=0.7,methods=0,l1=True,val_range=2,normalize="relu", pads=False,ssm_alpha=0.84)
            ...
            if EG_update1:
                ...
                C_loss,EG_loss,mmd_penalty = wali.forward(x=x,z=z,lamb=LAMBDA,beta1=0.36,beta2=0.12,gan=1,loss_type='reg-gp',methods=1,var_beta=var_beta,clip_beta=clip_beta)
                EG_loss.backward()
...
#             # print training statistics
            if curr_iter % 10 == 0:
                ...
                print(
                    ...
                    % (curr_iter, ITER, EG_losses[-1], C_losses[-1],mss_losses[-1],l1s[-1],MMD_EG_losses[-1],MMD_C_losses[-1],Recon_z_losses[-1],mmd_penaltys[-1],
                       EG_losses2[-1]))

                # plot reconstructed images and samples
                if curr_iter % 100 == 0:
                    wali.eval()
                    real_x, rect_x = init_x[:16], wali.reconstruct(init_x[:16]).detach_()
                    rect_imgs = torch.cat((real_x.unsqueeze(1), rect_x.unsqueeze(1)), dim=1)
                    rect_imgs = rect_imgs.view(32, NUM_CHANNELS, IMAGE_SIZE, IMAGE_SIZE).cpu()
...
#
            ...
            if curr_iter % (ITER // 20) == 0:
                torch.save(wali, mmds+'/celeba/%d/total_%d.pth' % (num_exp,curr_iter))
                torch.save(wali.get_G(),  mmds+'/celeba/%d/G_%d.pth' % (num_exp,curr_iter))
                torch.save(wali.get_C(),  mmds+'/celeba/%d/C_%d.pth' % (num_exp,curr_iter))
                torch.save(wali.get_E(),  mmds+'/celeba/%d/E_%d.pth' % (num_exp,curr_iter))
                torch.save(wali.get_MMDX(), mmds+'/celeba/%d/MMDX_%d.pth' % (num_exp,curr_iter))
                torch.save(wali.get_MMDZ(), mmds+'/celeba/%d/MMDZ_%d.pth' % (num_exp,curr_iter))
            #break
       # break
